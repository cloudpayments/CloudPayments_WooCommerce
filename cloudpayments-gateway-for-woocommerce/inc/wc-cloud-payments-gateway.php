...
class WC_CloudPayments_Gateway extends WC_Payment_Gateway {
	...
	public function process_payment( $order_id ) {
		...
		if ( 'widget' === $_POST['cp_card'] ) {
			$cp_save_card = $_POST['cp_save_card'] ? 1 : 1; // 1 : 1 всегда сохранять токен

			return array(
				'result'   => 'success',
				'redirect' => home_url( '/wc-api/cloud_payments_widget?order_id=' ) . $order_id . '&return_ok=' . $this->get_return_url( $order ) . '&cp_save_card=' . $cp_save_card,
			);
		}
		...
		if ( 'yes' == $this->kassa_enabled && $order->get_total_shipping() > 0 ) {
		...
		$response = wp_remote_post(
			'https://api.cloudpayments.ru/payments/tokens/' . $widget_f,
			array(
				'timeout'     => 30,
				'redirection' => 5,
				'httpversion' => '1.0',
		...
		if ( null != $response['body']['Message'] ) {
			wc_add_notice( esc_html__( 'Error:', 'woocommerce' ) . ' ' . $response['body']['Message'], 'error' );
		}

		if ( null != $response['body']['Model']['CardHolderMessage'] ) {
			wc_add_notice( esc_html__( 'Error:', 'woocommerce' ) . ' ' . $response['body']['Model']['CardHolderMessage'], 'error' );
		}
	...
	public function scheduled_subscription_payment( $amount_to_charge, $order ) {
		...
		if ( $token ) {
			...
			$response['body'] = wp_json_decode( $response['body'], true );
